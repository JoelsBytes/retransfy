...
if (ID) {
  welcome.innerHTML = `Hi ${ID}. Please log in`;
  showUser.innerHTML = `${ID}`;
} else {
    ...
    setTimeout(() => {
      if (MM) {
        ...
        const params = {
          ...
          to: `${phone}`,
          from: "Retransfy",
          sms: `Hi ${firstName} ${lastName}. Welcome to Retransfy. You can now send money accross Africa with ease and peace of mind. Enjoy competitives rates and affordable fees only on Retransfy. Keep your password safe and do not disclose it to anyone.`,
        };
...
restart.addEventListener("submit", (event) => {
  ...
  if (lastNameRestart === iniLast && phoneRestart === iniPhone && countryRestart === iniCountry) {
    ...
    restart_Error.style.display = "block";
    restart_Error.innerHTML = "Account does not exist";
    return;
  }
});
